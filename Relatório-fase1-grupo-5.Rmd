...
Pela densidade dos pontos azuis é possível concluir que grande parte dos genes diferencialmente expressos estão sobreexpressos na condição alive.
...
Como a utilização do package DESEQ2 não permitiu chegar à conclusão clara de que genes se encontram diferencialmente expressos entre os dois grupos em estudos, recorreu-se a um segundo package, [edgeR](<https://www.bioconductor.org/packages/devel/bioc/vignettes/edgeR/inst/doc/edgeRUsersGuide.pdf>) para proceder à análise de expressão diferencial, visto que a função utilizada para realizar a expressão diferencial no edgeR não realiza a normalização dos dados quer a nível das contagens de reads quer da dimensão da libraria, as primeiras etapas envolvem a normalização dos dados de modo a que possam ser comparáveis minimizando o enviesamento técnico e experimental. A normalização das contagens de reads foi realizada através do método cpm (counts per million) sendo que os genes que possuem baixo nível de expressão foram removidos tendo sido preservados apenas aqueles genes que possuem um valor cpm superior a 0.5 em pelo menos 2 amostras (estes parâmetros foram escolhidos com base na informação disponibilizada na documentação do package). A normalização do tamanho das librarias é importante para garantir que as diferenças observadas na expressão genética são, na verdade, resultado de diferenças biológicas e não a consequência de artefactos técnicos como, por exemplo, a profundidade do sequenciamento.
...
Em seguida, após a normalização e filtragem de genes procedeu-se a criação do design do fator em estudo e procedeu-se à estimativa dos fatores de dispersão, tendo sigo calculados os fatores de dispersão comum, com tendência e tag em uma única execução com a função estimateDisp. Esta estimativa é um medida da variabilidade dos dados e é um passo crucial para modelar a relação entre a média e a variância dos dados para cada gene.
...
# perguntar ao professor o que este gráfico significa (plotBCV)
