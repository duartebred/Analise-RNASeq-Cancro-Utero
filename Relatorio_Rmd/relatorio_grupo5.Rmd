...
Assim, este agrupamento dá informações sobre a similaridade entre os genes, permitindo identificar genes que são coexpressos e que podem estar envolvidos em vias metabólicas ou noutro processo similar.
...
À semelhança do que foi feito anteriormente, para os pacientes, com a finalidade de calcular a matriz euclidiana, procedeu-se à realização de testes estatísticos ttest para cada gene, na matriz de dados, determinando diferenças na expressão genética entre grupos experimentais.
De seguida, classificou-se os genes com base nos seu p-values. Posteriormente, selecionou-se os 30 genes com menores valores p, e criou-se uma nova matriz com as linhas correspondentes a esses genes.
Por fim, calculou-se a matriz de distâncias euclidiana entre esses genes, para análises de clustering.
...
Através da observação do gráfico é possível verificar que o ponto no gráfico que se assemelha a um “cotovelo” é o ponto K=4. A partir do ponto k=4, a adição de mais clusters não resulta numa redução significativa da WSS.
