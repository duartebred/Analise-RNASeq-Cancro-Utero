...
A coluna "vital_status", incorpora uma variável nominal  que representa a sobrevivência do paciente com cancro. Esta variável possui 3 categorias distintas: "Alive", "Dead", "NA".
...
Esta classificação está dividia em 4 estádios principais, nomeadamente, estádio I onde o tumor está confinado ao corpo uterino, estádio II onde já existe envolvimento do cérvix/colo uterino, estádio III que representa um atingimento de estruturas adjacentes do mesmo órgão, como, por exemplo, os gânglios linfáticos da zona pélvica. Por último, o estádio IV corresponde ao estado de doença em que já existem metástases noutros órgãos como por exemplo no intestino.
...
Em seguida, após a normalização e filtragem de genes, procedeu-se a criação do design da variável em estudo e procedeu-se à estimativa dos fatores de dispersão, tendo sido calculados os fatores de dispersão comum, com tendência e tag com a função estimateDisp.
...
A análise dos gráficos dos genes diferencialmente expressos de ambos os packages permite concluir que embora os packages tenham produzidos resultados ligeiramente diferentes a análise é bastante semelhante.
...
Este gene expressa a proteína ["claudin18"](https://www.uniprot.org/uniprotkb/P56856/entry) que, segundo um estudo realizado 2020, não se encontra expresso em tecidos saudáveis com a exceção do tecido da mucosa gástrica, sendo comummente expresso em vários tipos diferentes de cancro [5].
...
Segundo o estudo realizado por Bianco B et al. em 2020 [6] os genes mais comummente mutados neste tipo de cancro são PTEN(\>77%), PIK3CA (53%), PIK3R1 (37%), CTNNB1 (36%), ARID1A (35%), K-RAS (24%), CTCF (20%), RPL22 (12%), TP53 (11%), FGFR2 (11%) e ARID5B (11%).
...
Em seguida, recolheu-se os ids ensembl dos genes identificados pelo estudo e verificou-se se os genes apresentam expressão diferencial.
...
Uma análise mais detalhadas destes genes, revela que na realidade o gene ENSG00000121879.6, apresenta um comportamento semelhante em ambos os grupos, no entanto, o nível "Dead" apresenta maior variação dos resultados e, além disso, possui um menor número de amostras, e, por estas razões, o package identifica o gene como sendo diferencialmente expresso, uma vez que a variação leva a um aumento da média do grupo "Dead. Para o gene ENSG00000133703.13 os resultados parecem mais promissores, visto que, o grupo "Dead" apresenta resultados com menor variação, no entanto, não existe separação clara entre os dois grupos o que pode sugerir que se o dataset possuísse mais amostras para o grupo "Dead" o gene, muito possivelmente, não seria identificado como diferencialmente expresso. Por os motivos apresentados acima, não existe confiança de que, na realidade, este genes são diferencialmente expressos.
...
Por outras palavras, a análise de enriquecimento de pathways transforma dados de expressão genética em informações biologicamente relevantes, sendo iportante na compreensão dos mecanismos molecualres em diversas condições biológicas e doenças.
...
Os genes da análise de expressão diferencial foram ordenados de forma decrescente de acordo com o valor de "folg2FoldChange" e em seguida os genes que se encontram duplicados foram eliminados, uma vez que a redundância de genes produz resultados inesperados na análise de enriquecimento.
...
Segundo a informação disponível no site da [GSEA](https://www.gsea-msigdb.org/gsea/index.jsp) este conjunto de genes está associada ao aumento da resposta imunitária que provoca a rejeição de orgãos tansplantados.
...
Destacou-se também a via "HALLMARK_E2F_TARGETS", esta via está relacionada ao controlo do ciclo celular e à regulação genética por fatores de transcrição da família E2F, envolvendo genes que são ativados durante a fase G1 do ciclo celular que são essenciâncias para a progressão do ciclo celular.
No nosso caso de estudo esta via sofreu enriquecimento nas amostras do grupo Dead (valor NES positivo) o que indica um aumento da proliferação das células do cancro que juntamente com a diminuição da resposta imunitária possibilta o desenvolvimento descontrolado do tumor e o desenolvimento para um estádio da doença mais agressivo da doença
...
Conclui-se então que, embora a análise de expressão diferencial não produziu resultados que permitisse identificar genes que pudessem explicar o estado vital dos pacientes, a análise de enriquecimento permitiu identificar as vias metabólicas que se encontram alteradas e que ajudam a explicar o estado vital e permite compreender um pouco melhor o mecanismo que leva ao agravamento da doença
...
Com o objetivo de preparar os dados para o clustering, começa-se com a normalização dos dados de expressão gênica **(ddsSE)** usando o método DESeq.
Em seguida, os dados normalizados são convertidos numa matriz **(data_rna_UCEC_matrix)**.
Posteriormente, a matriz é transposta para que as linhas se tornem colunas e vice-versa **(data_rna_UCEC_transposed)**.
Essa transposição pode ser útil em análises posteriores, como por exemplo, quando se deseja ter genes como linhas e amostras como colunas.
...
Os clusters são representados pelos nós.
...
Após calcular a matriz Euclidiana, diferentes métodos podem ser usados para calcular a distância entre os clusters, como por exemplo: single linkage, complete linkage ou average linkage.
As 3 formas estão codificadas de seguida.
Além disso, podemos aplicar essa técnica agrupando genes ou pacientes.
...
Com a finalidade de calcular a matriz euclidiana, procedeu-se à realização de testes estatísticos t para cada gene, na matriz de dados, determinando diferenças na expressão genética entre grupos experimentais.
De seguida, classificam-se os genes com base nos valores p.
Posteriormente, selecionaram-se os 30 genes com menores valores p, e criou-se uma nova matriz com as linhas correspondentes a esses genes.
Por fim, calcula-se a matriz de distâncias euclidiana entre esses genes, para análises de clustering.

##### Por genes

Identificou-se os grupos de genes que têm padrões de expressão semelhantes entre os pacientes/amostras.
Assim, este agrupamento dá informações sobre a similaridade entre os genes, permitindo identificar genes que são coexpressos e podem até estar envolvidos em vias metabólicas ou outro processo semelhante.
...
Analisando os três métodos de medida de distância entre os clusters, podemos concluir que em todos se verifica a presença dos mesmos 30 genes mais diferencialmente expressos, proporcionando confiança nos resultados obtidos, uma vez que, as 3 abordagens distintas dão, em suma, a mesma divisão.
Além disso, poderíamos dividir estes 30 genes em 3 subgrupos e a partir daí explorar os genes associados.

##### Por pacientes

Identificou-se os grupos de pacientes que se assemlham com base no perfil de expressão genético.
...
Fazendo uma análise global dos três métodos de medida de distância entre os clusters, podemos concluir que em todos se verifica a presença dos mesmos 30 pacientes com um perfil genético mais diferenciado.
Todavia, a escolha de clusters nos três tipos de abordagens é mais subjetiva e complicada.
Desta forma, não conseguimos garantir uma divisão semelhante em clusters dos 30 pacientes, sendo este um aspecto que poderia causar dificuldade numa futura análise.

#### 2.2.k-means

Esta abordagem de clustering os dados de forma a minimizar as distâncias entre os pontos e centróides dos clusters.
Um método muito utilizado para auxiliar na determinação do número adequado de clusters é o “método do cotovelo”.
Este método envolve a análise da variação da soma dos quadrados intra-cluster (WSS) em relação ao número de clusters.
...
Através da observação do gráfico é possível verificar que o ponto no gráfico que se assemelha a um “cotovelo” é o ponto com K=4.A partir do ponto k=4, a adição de mais clusters não resulta numa redução significativa da WSS.
Partindo desse princípio, decidimos aplicar o algoritmo do Kmeans, tendo como referência o valor de 4 centróides, de modo a criar uma tabela que nos permitisse analisar a distribuição dos pacientes nos clusters em relação ao metadado estudado (vital_status).
...
Observando a tabela, é possível verificar que existem clusters que têm uma separação diferenciada no que toca ao metadado analisado (vital_status), cumprindo o principal propósito do k-means.
Todavia, existem clusters cuja separação não é a ideal.
