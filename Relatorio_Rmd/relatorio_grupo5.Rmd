...
pred_rf = predict(rf_model, newdata = data_set_treino[,1:600]) # não é o mais correto mas o dataset_treino dá erro
...
Com a análise das métricas dos dois modelos analisados conclui-se que os modelos apresentam eleveda precisão (cerca de 90%), no entanto estas métricas podem estar enviezadas pelo facto do serem geradas para o conjunto de teste, não sendo por isso muito confiáveis. Além disso, o número de amostras utilizados para a criação dos modelos é bastante reduzido o que pode resultar em modelos poucos precisos para a previsão de novos dados
